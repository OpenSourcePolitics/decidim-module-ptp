...
describe "Sorting projects" do
  ...
  shared_examples "ordering projects by selected option" do |selected_option|
    ...
    it "lists the projects ordered by selected option" do
      ...
      expect(page).to have_css("#projects .card__grid-grid .card__grid:first-child h3", text: translated(first_project.title))
      expect(page).to have_css("#projects .card__grid-grid .card__grid:last-child h3", text: translated(last_project.title))
    end
  end
